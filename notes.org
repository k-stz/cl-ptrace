...
** Machine Code Disassembly by hand
   ...
   the 2nd operand is "imm16/32". Which might just mean immediate.

   Makes sense given the 0xABCD value! So we compare the contents of rAX with the
   immediate value ABCD. Now lets look at the next intruction, it might be a condition
   branch:

   =2009cf058be474=

   First of, we see that is just a few bytes past the first instruction:
           8be4740000abcd3d
   2009cf058be474
   Such that our CMP instruction is 5 bytes in size: "0000abcd3d", or rather
   3D ABCD 0000
