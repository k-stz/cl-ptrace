...
* CFFI
  "Bindings [to C] are written in Lisp. They can be created at-will by Lisp programms. Lisp
  programmers can write new bindings and add them to the image (...)
  The oly foreig libray to load is the oe being wrapped- the one with pure C interface; no
...
** C - .a and .so files
  ...
  _takes place once, statically, during compliation_
...
** .a .so how does it matter to CFFI?
   Because only shared object files "*.so" can by dyncamically loaded into the
   Lisp image for binding creation

** creating .so libraries
   Only differs from regular compilationn in the command line switches
...
** TODO How can we create bindings for a function when all we did is load the binary shared object?
   ...
   for example =nm libtest.so= returns among other:
    ...
    There might be some useful info in there pertaining address ofset of particular
    functions. (And where to find functions of interest in the process' memory via ptrace!)
   
    For example "T" in the 2nd column means that the particular symbol is in the test/code section
