...
** =PTRACE_SYSCALL=
   will also continue execution of the child, but will stop every syscall invocation the
...
*** ASLR - Address Space Layout Randomization
    ...
    From what I could gather ALSR is only applied at process initialisation, so far, and
...
** disassembly doing it all again, a few months after hiatus..
  ...
  #x400 then in memory it will be laid out as 0 0 4 0 The first two zeros are the first
  ...
  and it is only 6 hex numbers apart, called peekdata should return some overlapping
  data!
  ...
  C0 = EAX
  and 01 = $01
  ... so the gdb output doesn't seem to intend to mirror the machine code byte sequence
