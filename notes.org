...
** =PTRACE_ATTACH=
   ...
   - In the background all the cases are handled (1) is it the init (2) current (3)
     already traced process, before permission to attach is granted.
   - "the child is removed from the task quere and its parent process field is changed
     (...). It is put to the quere again in such a position that *init* comes next to it*
   - (!)Interesting: _finally a =SIGSTOP= signal is delivered to it._ This could be useful
