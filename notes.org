...
** Registers
  ...
  These are not all, nor fixed. For excel just "SSE" may operate on 64 MMX and 128 XXM!
...
* disassembly hacking
  ...
  Example from (dspm)
   ...
   The addresses are thus aligned variably! The PUSH operation only needs a single byte!
...
** disassembly process hacking
   ...
   (TODO, test this again! isn't rip refering to next instruction and as such it the
   <next-instruction-offset> should be subtracted from the current rip-address?):
   so to refer to flag in this example
   ...
   TODO: test if changing it in a register will write the changed value back
         to memory? This might be an issue of modifying the register, like
...
** disassembly doing it all again, a few months after hiatus..
  ...
  Well after rediscovering that I already did a lot of good progress on this same
  issue some months ago before my hiatus on this project, calling
...
** Machine Code Disassembly by hand
   ...
   0x000000000000065b <+17>:	mov    eax,DWORD PTR [rip+0x2009f3]        # 0x201054 <x>
   0x0000000000000661 <+23>:	add    eax,0x1
   0x0000000000000664 <+26>:	mov    DWORD PTR [rip+0x2009ea],eax        # 0x201054 <x>
   0x000000000000066a <+32>:	mov    eax,DWORD PTR [rip+0x2009dc]        # 0x20104c <flag>
   0x0000000000000670 <+38>:	cmp    eax,0xabcd
   0x0000000000000675 <+43>:	je     0x65b <main+17>


*** Instruction Format

**** ModR/M Byte
     is the "operand-identifier" byte
     It can determine what register, or if a memory address is used
...
**** opcode 89: MOV memory/register, register  (move data from register to memory/register)
     1. Operand: "r/m16/32/64", geek64: *Evqp* (bold face means operand operand gets
        modified by instruction.

	'vqp'or '16/32/64' means word or dw depending on
        operand-size attribute, or qw promoted by "REX.W" in 64 bit mode.

	*E* or 'r/m' means that the ModR/M byte and specifies if the operand
	is a register or memory address.

	If it is a memory address, then it is calculated using a base register, index
        register, scalling factor, or a displacment
...
** Finally what does it matter to this project?
   ...
   than in what way is the above laid out in memory,
